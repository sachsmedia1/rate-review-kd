-- ============================================================================
-- IMPROVED QUICK MIGRATION - Mit Fallback für fehlende Daten
-- ============================================================================
...
SELECT id, slug as old_slug, customer_firstname, customer_lastname, city, 
       product_category, installation_date, is_published, created_at, customer_id
FROM reviews
WHERE slug ~ '-\d{7}$';

-- 2. Text-Bereinigung Funktion (verbessert)
...
WITH base_slugs AS (
  SELECT id, slug as old_slug,
    CASE 
      WHEN COALESCE(NULLIF(TRIM(city), ''), '') = '' THEN
        -- Fallback: Nutze customer_id wenn Stadt fehlt
        clean_slug_text(product_category) || '-' ||
        clean_slug_text(customer_lastname) || '-' ||
        customer_id || '-' ||
        EXTRACT(YEAR FROM installation_date)::text
      ELSE
        -- Normal: Mit Stadt
        clean_slug_text(product_category) || '-' ||
        clean_slug_text(customer_lastname) || '-' ||
